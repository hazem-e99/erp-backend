...
async function resetDatabase() {
  ...
  try {
    ...
    const collectionsToProcess = collections.filter(
      (c) => !keepCollections.includes(c.name) && c.name !== 'users',
    );
    ...
    for (const collection of collectionsToProcess) {
      ...
      console.log(
        `   ✅ Cleared ${collection.name}: ${result.deletedCount} documents deleted`,
      );
    }
    ...
    const keepUser = await usersCollection.findOne({ email: KEEP_USER_EMAIL });
    ...
    if (keepUser) {
      ...
      console.log(
        `   ℹ️  User ${KEEP_USER_EMAIL} not found, deleting all users and creating new one...`,
      );
    ...
    console.log(
      `   - Cleared collections: ${collectionsToProcess.map((c) => c.name).join(', ')}`,
    );
