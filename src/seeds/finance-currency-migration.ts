...
async function migrate() {
  ...
  const installmentModel = app.get<Model<Installment>>(
    getModelToken(Installment.name),
  );
  ...
  try {
    ...
    await paymentModel.updateMany({ baseAmount: { $exists: false } }, [
    ...
    await expenseModel.updateMany({ baseAmount: { $exists: false } }, [
      { $set: { baseAmount: '$amount' } },
    ]);
    ...
    await revenueModel.updateMany({ baseAmount: { $exists: false } }, [
      { $set: { baseAmount: '$amount' } },
    ]);
    console.log(
      `   ✅ Updated ${revenueResult.modifiedCount} revenue entries\n`,
    );
    ...
    await subscriptionModel.updateMany({ baseTotalPrice: { $exists: false } }, [
      { $set: { baseTotalPrice: '$totalPrice' } },
    ]);
    console.log(
      `   ✅ Updated ${subscriptionsResult.modifiedCount} subscriptions\n`,
    );
    ...
    await installmentModel.updateMany({ baseAmount: { $exists: false } }, [
      { $set: { baseAmount: '$amount' } },
    ]);
    console.log(
      `   ✅ Updated ${installmentsResult.modifiedCount} installments\n`,
    );
    ...
    await transactionModel.updateMany({ baseAmount: { $exists: false } }, [
      { $set: { baseAmount: '$amount' } },
    ]);
    console.log(
      `   ✅ Updated ${transactionsResult.modifiedCount} transactions\n`,
    );
    ...
    if (
      ...
      console.log(
        `   ⚠️  Warning: ${paymentsCheck + expensesCheck + revenueCheck + subscriptionsCheck + installmentsCheck} records still missing currency fields\n`,
      );
    }
  } catch (error) {
