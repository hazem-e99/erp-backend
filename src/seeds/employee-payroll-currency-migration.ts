...
async function migrate() {
  try {
    ...
    console.log(
      `   ✓ Updated ${employeesResult.modifiedCount} employees with currency fields`,
    );
    console.log(
      `   ✓ Calculated base amounts for ${employeeUpdates} employees`,
    );
    ...
    for (const payroll of payrolls) {
      ...
      const baseCommissions =
        Math.round(commissions * exchangeRate * 100) / 100;
      const baseDeductions = Math.round(deductions * exchangeRate * 100) / 100;
      const baseOvertimePay =
        Math.round(overtimePay * exchangeRate * 100) / 100;
      const baseMaxKpi = Math.round(maxKpi * exchangeRate * 100) / 100;
    ...
    console.log(
      `   ✓ Updated ${payrollsResult.modifiedCount} payrolls with currency fields`,
    );
