...
export class HrService {
  ...
  async getAttendanceOverview(query: any = {}) {
    ...
    const records = await this.attendanceModel
      ...
      .populate({
        path: 'employeeId',
        select: 'name employeeId department departments',
      });

    const checkedInIds = new Set(
      records.map((r) => r.employeeId?._id?.toString()),
    );
    ...
    const result = allEmployees.map((emp) => {
      ...
      return {
        employee: {
          ...
          department: emp.departments?.[0] || 'N/A',
        },
    ...
    if (employeeId)
      filtered = result.filter((r) => r.employee._id.toString() === employeeId);
  ...
  async getAnalytics(query: any = {}) {
    ...
    if (department)
      empFilter.$or = [{ department }, { departments: department }];

    let employees = await this.employeeModel
      .find(empFilter)
      .select('name employeeId department departments');
    if (employeeId)
      employees = employees.filter((e) => e._id.toString() === employeeId);
    ...
    const employeeReports = employees.map((emp) => {
      ...
      const totalOvertime = empRecords.reduce(
      ...
      return {
        ...
        attendancePercentage:
          expectedDays > 0
            ? parseFloat(((present / expectedDays) * 100).toFixed(1))
            : 0,
        avgHoursPerDay:
          present > 0 ? parseFloat((totalHours / present).toFixed(2)) : 0,
      };
    });
    ...
    const avgAttendance =
      employeeReports.length > 0
        ? parseFloat(
            (
              employeeReports.reduce((s, r) => s + r.attendancePercentage, 0) /
  ...
  async getAttendanceTrend(query: any = {}) {
    ...
    const totalEmps = await this.employeeModel.countDocuments({
      status: 'active',
    });
    const records = await this.attendanceModel.find({
      date: { $gte: startDate, $lt: endDate },
