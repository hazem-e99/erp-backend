...
export class ReminderSchedulerService implements OnModuleInit {
  ...
  onModuleInit() {
    ...
    console.log(
      `⏰ Checking reminders every 5 minutes (Debug mode: ${this.DEBUG_MODE})`,
    );
  }
    ...
    try {
      ...
      for (const { reminder, period } of pendingReminders) {
        ...
        if (user && user.email) {
          ...
          try {
            await this.emailService.sendReminderEmail(user.email, subject, {
            ...
            console.log(
              `   ✅ Sent "${reminder.title}" to ${user.email} (${period})`,
            );
          } catch (emailError) {
            console.error(
      ...
      const monthlyReminders =
        await this.remindersService.getMonthlyRecurringReminders(
          this.DEBUG_MODE,
        );
      ...
      for (const { reminder } of monthlyReminders) {
        ...
        if (user && user.email) {
          ...
          try {
            ...
            console.log(
              `   ✅ Sent monthly "${reminder.title}" to ${user.email} (day ${reminder.monthlyDay})`,
            );
          } catch (emailError) {
            console.error(
              `   ❌ Failed to send monthly to ${user.email}:`,
              emailError.message,
            );
          }
        } else {
